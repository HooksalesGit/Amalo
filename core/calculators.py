...
def monthly_payment(principal, annual_rate_pct, term_years):
    L = nz(principal); r = nz(annual_rate_pct)/100/12; n = int(nz(term_years)*12)
    if n<=0: return 0.0
    if abs(r) < 1e-9: return L/n
    return (r*L)/(1-(1+r)**(-n))

def principal_from_payment(payment, annual_rate_pct, term_years):
    P = nz(payment); r = nz(annual_rate_pct)/100/12; n = int(nz(term_years)*12)
    if n<=0: return 0.0
...
def w2_totals(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame(columns=['BorrowerID','BaseMonthly','VariableMonthly','QualMonthly','W2_DecliningVarFlag'])
    out = df.copy()
    def base_monthly(r):
        t = str(r.get('PayType','')).lower()
        if t=='salary': return nz(r.get('AnnualSalary'))/12
        if t=='hourly': return nz(r.get('HourlyRate'))*nz(r.get('HoursPerWeek'))*52/12
        return 0.0
    out['BaseMonthly']=out.apply(base_monthly, axis=1)
    out['VarTotal']=out[['OT_YTD','Bonus_YTD','Comm_YTD']].fillna(0).sum(axis=1)+out[['OT_LY','Bonus_LY','Comm_LY']].fillna(0).sum(axis=1)
    out['VarMonths']=out[['Months_YTD','Months_LY']].fillna(0).sum(axis=1).replace(0, pd.NA)
    out['VariableMonthly']=(out['VarTotal']/out['VarMonths']).fillna(0)
    out['YOY_Var_Annualized']=((out[['OT_YTD','Bonus_YTD','Comm_YTD']].fillna(0).sum(axis=1))/(out['Months_YTD'].replace(0,pd.NA))*12).fillna(0)
    out['DecliningVarFlag']=(out['OT_LY'].fillna(0)+out['Bonus_LY'].fillna(0)+out['Comm_LY'].fillna(0))>(1.2*out['YOY_Var_Annualized'])
    ...
    agg=out.groupby('BorrowerID',dropna=False).agg(BaseMonthly=('BaseMonthly','sum'),
        VariableMonthly=('VariableMonthly','sum'),QualMonthly=('QualMonthly_row','sum'),
        W2_DecliningVarFlag=('DecliningVarFlag','any')).reset_index()
    return agg
...
def sch_c_totals(df: pd.DataFrame)->pd.DataFrame:
    ...
    out['AdjustedAnnual']=(nz_series(out.get('NetProfit'))+nz_series(out.get('Nonrecurring'))+nz_series(out.get('Depletion'))+
...
def k1_totals(df: pd.DataFrame)->pd.DataFrame:
    ...
    out=df.copy()
    out['AdjustedAnnual']=(nz_series(out.get('Ordinary'))+nz_series(out.get('NetRentalOther'))+nz_series(out.get('GuaranteedPmt'))+
        nz_series(out.get('Nonrecurring'))+nz_series(out.get('Depreciation'))+nz_series(out.get('Depletion'))+
        nz_series(out.get('AmortCasualty'))-nz_series(out.get('NotesLT1yr'))-nz_series(out.get('NonDed_TandE')))
...
def ccorp_totals(df: pd.DataFrame)->pd.DataFrame:
    ...
    out['AdjustedAnnual']=(nz_series(out.get('TaxableIncome'))-nz_series(out.get('TotalTax'))+nz_series(out.get('Nonrecurring'))+
...
def rentals_policy(df: pd.DataFrame, method='ScheduleE', subject_pitia=0.0, subject_market_rent=0.0):
    if df is None or df.empty:
        return pd.DataFrame(columns=['BorrowerID','Rental_Monthly'])
    out=df.copy()
    if method=='ScheduleE':
        out['NetAnnual']=nz_series(out.get('Rents'))-nz_series(out.get('Expenses'))+nz_series(out.get('Depreciation'))
...
def other_income_totals(df: pd.DataFrame)->pd.DataFrame:
    ...
    out=df.copy()
    out['QualMonthly']=nz_series(out.get('GrossMonthly'))*(1.0+nz_series(out.get('GrossUpPct'))/100)
...
def combine_income(num_borrowers:int, w2=None, schc=None, k1=None, c1120=None, rentals=None, other=None)->pd.DataFrame:
    base=pd.DataFrame({'BorrowerID':list(range(1,num_borrowers+1))})
    def mergecol(L,R,col):
        if R is None or R.empty: R=pd.DataFrame(columns=['BorrowerID',col])
    ...
    for c in ['W2_Monthly','SchC_Monthly','K1_Monthly','C1120_Monthly','Rental_Monthly','Other_Monthly']:
        out[c]=out[c].fillna(0.0)
    out['TotalMonthlyIncome']=out[['W2_Monthly','SchC_Monthly','K1_Monthly','C1120_Monthly','Rental_Monthly','Other_Monthly']].sum(axis=1)
...
def compute_ltv(purchase_price, base_loan):
    if nz(purchase_price)==0: return 0.0
    return 100.0*nz(base_loan)/nz(purchase_price)

def conventional_mi_factor(ltv, fico_bucket, mi_table):
    if ltv>=97: return mi_table.get(">=97",0.90)
    if 95<=ltv<97: return mi_table.get("95-97",0.62)
    if 90<=ltv<95: return mi_table.get("90-95",0.40)
    if 85<=ltv<90: return mi_table.get("85-90",0.25)
...
def va_funding_fee_pct(first_use, down_pct, table):
    if first_use:
        if down_pct>=10: return table.get("first_10+",1.25)
        if down_pct>=5: return table.get("first_5_10",1.50)
        return table.get("first_0_5",2.15)
    else:
        if down_pct>=10: return table.get("subseq_10+",1.25)
        if down_pct>=5: return table.get("subseq_5_10",1.50)
        return table.get("subseq_0_5",3.30)

def usda_guarantee_pct(table): return table.get("guarantee_pct",1.0)
def usda_annual_fee_pct(table): return table.get("annual_pct",0.35)

def apply_program_fees(program, purchase_price, base_loan, dp_amt, rate_pct, term_years,
                       conv_mi_tbl, fha_tables, va_tbl, usda_tbl, finance_upfront, first_use_va, fico_bucket):
...
def piti_components(program, purchase_price, base_loan, rate_pct, term_years,
                    tax_rate_pct, hoi_annual, hoa_monthly,
                    conv_mi_tbl, fha_tables, va_tbl, usda_tbl,
                    finance_upfront, first_use_va, fico_bucket):
    adj=apply_program_fees(program, purchase_price, base_loan, purchase_price-base_loan,
                           rate_pct, term_years, conv_mi_tbl, fha_tables, va_tbl, usda_tbl,
                           finance_upfront, first_use_va, fico_bucket)
    pi=monthly_payment(adj['adjusted_loan'], rate_pct, term_years)
    taxes=nz(purchase_price)*nz(tax_rate_pct)/100/12
    hoi=nz(hoi_annual)/12; hoa=nz(hoa_monthly)
    total=pi+taxes+hoi+hoa+adj['mi_monthly']
    return {"pi":pi,"taxes":taxes,"hoi":hoi,"hoa":hoa,"mi":adj['mi_monthly'],"total":total,
            "adjusted_loan":adj['adjusted_loan'],"ltv":adj['ltv'],"upfront_amt":adj['upfront_amt']}
...
def max_affordable_pi(total_income, other_liabilities, taxes_ins_hoa_mi, target_fe_pct, target_be_pct):
    inc=nz(total_income)
